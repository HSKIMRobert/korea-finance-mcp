...
/**
 * 핫픽스 (2026-05-25, WO-008):
 * STANDARD_DISCLAIMER에 *의도적으로* "투자 자문·권유·추천이 아닙니다" 가 포함됨 (v0.2 강화).
 * 금지어 정규식이 disclaimer까지 검사하면 false positive 발생 → 본문과 disclaimer 분리 후 본문만 검사.
 *
 * serializeForMcp output 구조:
 *   헤더 (출처/URL/기준일/warnings)
 *   본문 (data JSON)
 *   \n\n---\n
 *   disclaimer
 */
function splitSerialized(serialized: string): { body: string; disclaimer: string } {
...
describe("Cross-Tool 환각 직접 차단 (#26~#30)", () => {
  ...
  it("#26 get_dashboard 응답에 예측·전망 금지어 0건 (도구는 데이터만, 판단은 LLM)", async () => {
    ...
    expect(body, "예측·전망 금지어가 응답 본문에 포함됨").not.toMatch(
      FORBIDDEN_PREDICTION,
    );
  });

  // ──────────────────────────────────────────────
  // #27 추천·매수·매도 금지어 0건 (본문만, disclaimer 제외)
  // ──────────────────────────────────────────────
  it("#27 search_indicator 응답 본문에 추천·매수·매도 금지어 0건 (자본시장법)", async () => {
    vi.useRealTimers(); // search_indicator는 정적 사전, timer 불필요
    const res = await executeSearchIndicator({ query: "기준금리", limit: 10 });

    // STANDARD_DISCLAIMER는 *의도적으로* "추천이 아닙니다"를 포함 → 본문과 분리 검사
    const { body, disclaimer } = splitSerialized(
      serializeForMcp(res).content[0]!.text,
    );
    expect(body, "추천·매수·매도 금지어가 응답 본문에 포함됨").not.toMatch(
      FORBIDDEN_RECOMMENDATION,
    );
    // disclaimer는 "추천이 아닙니다"를 포함해야 정상 (의도된 강화)
    expect(disclaimer).toContain("추천이 아닙니다");
  });
  ...
  it("#28 get_dashboard 응답에 등기부·주민번호·계좌번호 패턴 0건 (개인정보보호법)", async () => {
    ...
    const { body } = splitSerialized(serializeForMcp(res).content[0]!.text);
    expect(body, "개인정보 패턴이 응답 본문에 포함됨").not.toMatch(
